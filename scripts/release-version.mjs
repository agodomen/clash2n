/**
 * CLI tool to update version numbers in package.json, client/Cargo.toml, client/Cargo.lock, and client/tauri.conf.json.
 *
 * Usage:
...
async function updateCargoVersion(newVersion) {
  const _dirname = process.cwd()
  const cargoTomlPath = path.join(_dirname, 'client', 'Cargo.toml')
  try {
...
async function updateCargoLockVersion(newVersion) {
  const _dirname = process.cwd()
  const cargoLockPath = path.join(_dirname, 'client', 'Cargo.lock')
  const versionWithoutV = newVersion.startsWith('v')
    ? newVersion.slice(1)
    : newVersion
...
async function updateTauriConfigVersion(newVersion) {
  const _dirname = process.cwd()
  const tauriConfigPath = path.join(_dirname, 'client', 'tauri.conf.json')
  try {
